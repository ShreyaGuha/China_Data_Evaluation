...
reader.fn <- function( file_in ){
  ...
  if (ncol(dat_in) == 7)
  {
    ...
    # return dataset
    return( dat_day[,.( sta.ID, date, hr, variable, hr_mean, N)])
...
#save datasets
write.fst(in_data_daily, file = "D:/Professional/Projects/China_obs/Haotian/Haotian_ozone_hourly.csv", row.names = F)

#read data
in_data_daily <- read.fst("D:/Professional/Projects/China_obs/Haotian/Haotian_ozone_hourly.csv")
