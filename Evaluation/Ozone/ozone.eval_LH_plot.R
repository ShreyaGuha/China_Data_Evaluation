...
chap_data_obs <- fread("D:/Professional/Projects/China_obs/Haotian/CHAP_and_obs.csv")

#merge the datasets
obs_rasterfields_all <- 
  merge( tap_data_obs, chap_data_obs,
         by = c( 'station.name', 'year', 'ozone_obs'),
         all = TRUE) 
...
obs_rasterfields_all.m <- 
  melt( obs_rasterfields_all,
        id.vars = c( 'year', 'station.name', 'ozone_obs'),
...
gg_scatter <- 
  ggplot( obs_rasterfields_all,
          aes( x = ozone_obs,
               y = ozone_tap)) +
  scale_y_continuous(sec.axis ~ ozone_chap) +
...
ggplot( obs_rasterfields_all.m[year <= 2019],
        aes( x = ozone_obs,
             y = ozone_modeled,
             color = model)) + 
  geom_abline( slope = 1, intercept = 0) + 
...
ggsave( 'D:/Professional/Projects/China_obs/Haotian/ozone_scatter_chap_plot.jpeg',
        gg_scatter,
        height = 6, width = 6, scale = 1.2)
...
#save data
fwrite(eval_out_tap, file = "tap_eval.csv")
fwrite(eval_out_chap, file = "chap_eval.csv")
fwrite(eval_out, file = "o3_eval.csv")
...
# plot the evaluation
ggplot( eval_out.m[ model %in% c( 'ozone_tap', 'ozone_chap')],
        aes( x = year,
             y = value,
             color = model,
             group = model)) + 
